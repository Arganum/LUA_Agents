...
minGab = 20.0              -- the minimum gab kept at complete standstill in meters 2.0m
...
function initAgent(x, y, id, macroFactor, timeResolution, speed)
    ...
    --height, width = l_getEnvironmentSize() Brugt tidligere?
    l_debug("Width: " .. width .. " Height: " .. height)

    l_debug( "DistTorus: " .. distTorus(1990, 50, posX, posY) )
...
function handleEvent(origX, origY, origID, origDesc, origTable)
    -- make a response:
    l_debug(ID.." : "..origDesc )
    if origDesc == "infoIDM" and origID ~= ID then
        load("ctable="..origTable)()
        tempGab = distTorus(origX, origY, posX, posY) - ctable[1] -- - ctable[1] -- Distance to Leading Car - the Leading Car Lenght
        l_debug( "DistCar: " .. tempGab )
        if origX > posX and posX + 40 > origX then
            newGab = tempGab
        end
    end
    --distTorus(origX, origY, Explorer_PosX, Explorer_PosY)
    return 0,0,0,"null"
end
...
function distTorus(refPosX, refPosY, desPosX, desPosY)
    ...
    width, height = l_getEnvironmentSize() --Omvent?
    width = width + 1
    height = height + 1
